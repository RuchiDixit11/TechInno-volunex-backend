...
router.get(
  ...
  async (req, res) => {
    try {
      const records = await User.find({ 'user_type': 2 }, { fullname: 1, gender: 1, dob: 1, phone: 1, address: 1, city: 1, state: 1, zip: 1, skills: 1, volunteer_experience: 1, languages_spoken: 1, emergency_contact: 1, short_bio: 1 });
      res.json({ msg: 'Volunter get successfully', data: records });
    } catch (error) {
      res.status(500).json({ msg: 'Server error', error });
    }
...
router.get(
  ...
  async (req, res) => {
    try {
      const records = await User.find({ 'user_type': 1 });
      res.json({ msg: 'Org get successfully', data: records });
    } catch (error) {
      res.status(500).json({ msg: 'Server error', error });
    }
  }
...
router.get(
  ...
  async (req, res) => {
    try {
      const org_count = await User.find({ 'user_type': 1 }).count();
      const vol_count = await User.find({ 'user_type': 2 }).count();
      const camp_count = await Event.find().count();
      const records = {
        org_count: org_count,
        vol_count: vol_count,
